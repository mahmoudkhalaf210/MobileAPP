...
namespace Snap.Application.Orders.Services
{
    ...
    ///
    /// The realtime push goes through <see cref="IOrderRealtimeNotifier"/> — an
    /// Application-owned abstraction with no knowledge of Hub/HubContext/SignalR.
    /// The concrete SignalR adapter lives at the Snap.API composition boundary
    /// (it needs the OrdersHubV2 Hub type, which must live in the presentation
    /// project), but this class itself is fully transport-agnostic.
    ///
    /// Registered in place of the concrete OrderNotificationService in
    /// AddSnapApplication(), so every existing caller (OrderService,
    /// ScheduledRideProcessorService) gets both behaviors automatically without a
    /// single line of those files changing.
    /// </summary>
    public sealed class OrderNotificationServiceV2Decorator : IOrderNotificationService
    {
        ...
        private Task PushUser(string userId, string eventName) =>
            _realtime.NotifyUserAsync(userId, eventName);

        private Task PushAll() =>
            _realtime.NotifyAllAsync();
